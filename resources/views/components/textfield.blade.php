<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label {{ $errors->has($inputName) ? 'is-invalid' : '' }} {{ $class or '' }}">
	<input class="mdl-textfield__input {{ $inputClass or '' }}" type="{{ $type or 'text' }}" {{ isset($pattern) ? 'pattern="'.$pattern.'"' : '' }} name="{{ $inputName }}" id="{{ $id or $inputName }}" value="{{ old($inputName, $value) }}" {{ $disabled or '' }}>
